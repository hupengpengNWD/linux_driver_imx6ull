...
static int __init drv_led_init(void){
	...
	for (i = 0; i < LED_NUM; i++){
		device_create(drv_led_class_ptr, NULL, MKDEV(drv_led_major, i), NULL, "drv_led_module_hpp_%d", i); 
	}
	led_imx6ull_operations_ptr = hal_led_imx6ull_get_operations();
...
static void __exit drv_led_exit(void){
	...
	for (i = 0; i < LED_NUM; i++){
		device_destroy(drv_led_class_ptr, MKDEV(drv_led_major, i)); 
	}

	device_destroy(drv_led_class_ptr, MKDEV(drv_led_major, 0));
